...
def run_real_civitai_download():
    ...
    if not config.host or config.host in ("localhost", "127.0.0.1"):
        print(f"COMANI_HOST ({config.host}) not set to a remote server, skipping real download test")
        return
    ...
    with connect_node(
        ...
        if not node.exec_shell("aria2c --version").ok:
            ...
            if not node.exec_shell("aria2c --version").ok:
                print("Error: Failed to install aria2c on remote node")
                sys.exit(1)
        ...
        if not downloader.file_exists(remote_path):
            print(f"Error: File {remote_path} does not exist after download")
            sys.exit(1)
        ...
        if downloader.is_html_file(remote_path):
            print(f"Error: Downloaded file {remote_path} is an HTML file")
            sys.exit(1)
